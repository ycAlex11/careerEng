...
- Before ending a supervising turn, ensure a user-authorized Desktop heartbeat monitors the current batch in the main task. Read `monitor_policy` from CareerEng, reuse/update the existing monitor, and persist the local read cursor. No new monitor is needed for an already completed batch. Urgent events additionally use the worker relay below; a child's final response is not the durable event inbox.
- After reporting waiting-user, failure, or completion, the worker calls `careereng_claim_urgent_notification` with its current control epoch. On `send_required`, call Desktop `send_message_to_thread` with exactly the returned target and message, then record actual transport acceptance/failure with `careereng_record_urgent_notification_send`. A transport success is not main-task receipt or user presentation. Missing Desktop tools or send failures must not discard events or use private sockets/another App Server. Respect `retry_after`, allow at most three attempts, and leave remaining work to periodic polling rather than keeping a worker alive indefinitely. Never acknowledge user presentation from the child.
- When an urgent signal arrives, the main task reads the indicated CareerEng batch/site notifications and summarizes pending results in a non-empty final reply, not commentary alone. On its next turn it acknowledges those delivery IDs with the previously rendered final text and `presentation_channel="final"`. Retain pending receipts across turns; on completion allow one receipt-only heartbeat wake before pausing the monitor. A duplicate signal with no pending notification is silent. A notification does not authorize restarting a cancelled batch. Ordinary polling always follows the configurable interval; user input is processed immediately rather than waiting for a poll.
