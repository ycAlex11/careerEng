...
class JobFlow:
    ...
    def __init__(
        ...
        *,
        ...
        # Compatibility for callers/tests while storage migration is pending.
        self.job_planning_store = self.applications.planning_store
        self.continuation_registry = ContinuationRegistry()
        self.continuation_registry.register(
            scope="job_batch",
            phase="apply",
    ...
    def _run_job_rows(self, site_key: str, batch_id: str) -> list[dict[str, Any]]:
        return self.applications.run_job_rows(site_key, batch_id)

    @staticmethod
    def _fallback_run_job_identity(row: dict[str, Any]) -> str:
        return ApplicationPlanningService.fallback_run_job_identity(row)

    def _run_job_identity(self, site_key: str, row: dict[str, Any]) -> str:
        return self.applications.run_job_identity(site_key, row)

    def _merged_run_job_rows(self, site_key: str, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return self.applications.merged_run_job_rows(site_key, rows)

    def _merged_run_job_rows_for_batch(self, site_key: str, batch_id: str) -> list[dict[str, Any]]:
        return self.applications.merged_run_job_rows_for_batch(site_key, batch_id)

    @staticmethod
    def _terminal_application_status(row: dict[str, Any]) -> str:
        return ApplicationPlanningService.terminal_application_status(row)

    def _apply_counters_from_run(self, site_key: str, batch_id: str) -> dict[str, int]:
        return self.applications.apply_counters_from_run(site_key, batch_id)

    @staticmethod
    def _apply_counter_payload(counters: dict[str, int]) -> dict[str, int]:
        return ApplicationPlanningService.apply_counter_payload(counters)
    ...
    def _write_retrieval_snapshot(self, *, site_key: str, batch_id: str, current: dict[str, Any]) -> dict[str, Any]:
        return self.applications.write_retrieval_snapshot(site_key=site_key, batch_id=batch_id, current=current)

    def _latest_search_snapshot_id(self, *, site_key: str, batch_id: str) -> str:
        return self.applications.latest_search_snapshot_id(site_key=site_key, batch_id=batch_id)

    def _decision_context_hash(self, site_key: str) -> str:
        return self.applications.decision_context_hash(site_key)

    def _decision_context_versions(self, site_key: str) -> dict[str, str]:
        return self.applications.decision_context_versions(site_key)

    def _normalize_history_for_apply_plan(self, *, site_key: str, batch_id: str) -> dict[str, Any]:
        return self.applications.normalize_history_for_apply_plan(
            site_key=site_key,
            batch_id=batch_id,
            on_needs_review=lambda callback_site_key, callback_batch_id, count: self._create_history_normalization_card(
                site_key=callback_site_key,
    ...
    def _ensure_apply_plan(
        ...
        *,
        ...
        return self.applications.ensure_apply_plan(
            site_key=site_key,
            batch_id=batch_id,
            session_id=session_id,
            turn_id=turn_id,
            on_history_normalization_needs_review=lambda callback_site_key, callback_batch_id, count: self._create_history_normalization_card(
                site_key=callback_site_key,
                batch_id=callback_batch_id,
                count=count,
            ),
    ...
    def _apply_plan_counts(plan: dict[str, Any]) -> dict[str, int]:
        return ApplicationPlanningService.apply_plan_counts(plan)

    @staticmethod
    def _aggregate_apply_status(counters: dict[str, int]) -> str:
        return ApplicationPlanningService.aggregate_apply_status(counters)
    ...
    def _seal_apply_row_terminal(
        ...
        *,
        ...
        self.applications.seal_apply_row_terminal(
            site_key=site_key,
            batch_id=batch_id,
            session_id=session_id,
            turn_id=turn_id,
    ...
    def _apply_site_jobs(
        ...
        *,
        ...
        if apply_plan:
            current = self._with_apply_plan(current, apply_plan)
    ...
    def _with_apply_plan(self, current: dict[str, Any], apply_plan: dict[str, Any]) -> dict[str, Any]:
        """Attach persisted plan metadata without deciding any plan item."""
    ...
    def continue_external_phase_sequence(
        self,
        *,
        site_key: str,
        batch_id: str,
        ...
        """Consume one generic external-agent completion signal.

        The browser service emits the signal but does not call this method.
        This workflow boundary translates a completed declared sequence into the
        next existing career-domain capability.
        """
        ...
        if not isinstance(current, dict):
            return {"handled": False, "reason": "missing_site", "batch": batch}
        operation = self._normalize_operation(str(batch.get("operation") or ""))
        expected_phase = self._terminal_phase_for_operation(operation)
        if str(terminal_phase or "") == "apply":
            return self._continue_external_apply_target(
                batch=batch,
                site_key=site_key,
                current=current,
                session_id=str(session_id or batch.get("session_id") or "cli:default"),
                turn_id=str(turn_id or batch.get("turn_id") or ""),
            )
        if str(terminal_phase or "") != expected_phase:
            return {
                "handled": False,
                "reason": "unexpected_terminal_phase",
                "expected_phase": expected_phase,
                "terminal_phase": str(terminal_phase or ""),
                "batch": batch,
            }

        effective_session_id = str(session_id or batch.get("session_id") or "cli:default")
        effective_turn_id = str(turn_id or batch.get("turn_id") or "")
        browser_session = self.site_tools.site_store.load_browser_session(site_key)
        current_url = str(browser_session.get("last_known_url") or current.get("current_url") or current.get("entry_url") or "")

        if operation == self.OPERATION_APPLICATION_STATUS_REVIEW:
            updated = {
                **current,
                "status": "completed",
                "reason_tag": "agent_bridge_phase_sequence_completed",
                "message": "External agent completed application status review.",
        ...
        retrieved_rows = self._merged_run_job_rows_for_batch(site_key, batch_id)
        retrieve = dict(current.get("retrieve") or {})
        retrieve.update({"status": "done", "count": len(retrieved_rows)})
        updated = {
            **current,
            "status": "running",
            "reason_tag": "agent_bridge_phase_sequence_completed",
            "message": "External agent completed job retrieval.",
            "current_phase": terminal_phase,
            "current_url": current_url,
            "retrieve": retrieve,
        }
        ...
        if snapshot:
            ...
            updated = {**updated, "retrieve": retrieve}

        allow_apply = bool(batch.get("apply_requested")) and self.ENABLE_BROWSER_APPLY_PHASE
        if not allow_apply:
            updated["status"] = "completed"
            updated["apply"] = {**dict(updated.get("apply") or {}), "status": "skipped"}
            return self._save_external_phase_completion(batch, site_key=site_key, updated=updated)
        ...
        updated = self._with_apply_plan(updated, apply_plan)
        updated = self._start_next_external_apply_target(
            site_key=site_key,
            current=updated,
            batch_id=batch_id,
            session_id=effective_session_id,
    ...
    def _continue_external_apply_target(
        ...
        *,
        ...
        active_job_id = str(apply.get("active_target_job_id") or "").strip()
        if not active_job_id:
            return {
                "handled": False,
                "reason": "missing_active_apply_target",
                "batch_id": str(batch.get("batch_id") or ""),
            }
        rows_by_job_id = {
            str(row.get("job_id") or ""): row
            for row in self._run_job_rows(site_key, str(batch.get("batch_id") or ""))
            if isinstance(row, dict)
        }
        ...
        self._record_run_local_proposal_validation(
            site_key=site_key,
            batch_id=str(batch.get("batch_id") or ""),
            job_row=target_row,
        )
        counters = self._apply_counters_from_run(site_key, str(batch.get("batch_id") or ""))
        apply.pop("active_target_job_id", None)
        updated = {
            **current,
            "status": "running",
            "reason_tag": "agent_bridge_apply_target_completed",
            "message": "External agent completed the current apply-plan target.",
            "current_phase": "apply",
            "apply": {
                **apply,
                "status": "running",
    ...
    def _start_next_external_apply_target(
        self,
        *,
        site_key: str,
        current: dict[str, Any],
        ...
        if not pending_rows:
            ...
            return {
                **current,
                "status": "completed",
                "reason_tag": "apply_plan_completed_without_browser_target",
                "message": "All apply-plan items are already terminal.",
                "apply": {**dict(current.get("apply") or {}), "status": "done", **self._apply_counter_payload(counters)},
            }
        ...
        if not job_id or not job_url:
            if job_id:
                ...
                return self._start_next_external_apply_target(
                    site_key=site_key,
                    current=current,
                    batch_id=batch_id,
                    session_id=session_id,
            ...
            return {
                **current,
                "status": "failed",
                "reason_tag": "apply_target_identity_missing",
                "message": "The apply plan contains a target without a stable job id.",
            }
        ...
        if str(getattr(result, "status", "") or "") == "blocked" and is_agent_bridge_reason(
            str(getattr(result, "reason_tag", "") or "")
        ):
            counters = self._apply_counters_from_run(site_key, batch_id)
            return {
                **current,
                "status": "running",
                "reason_tag": str(getattr(result, "reason_tag", "") or "agent_bridge_required"),
                "message": str(getattr(result, "message", "") or "External agent apply work order is ready."),
                "current_phase": "apply",
                "current_url": str(getattr(result, "current_url", "") or job_url),
                "trace_ref": str(getattr(result, "trace_ref", "") or ""),
                "apply": {
                    **dict(current.get("apply") or {}),
                    "status": "running",
                    "active_target_job_id": job_id,
                    **self._apply_counter_payload(counters),
                },
            }
        return self._apply_result_to_site_row(result=result, existing=current, batch_id=batch_id)

    def _save_external_phase_completion(
        self,
        batch: dict[str, Any],
        *,
        site_key: str,
    ...
    def continue_item_loop(
        ...
        *,
        ...
        return self.continuation_registry.resume(
            ContinuationRequest(
                scope="job_batch",
                phase=phase,
    ...
    def _resume_apply_item_loop(self, request: ContinuationRequest) -> dict[str, Any]:
        """Bridge the generic continuation contract to application-domain work."""
    ...
    def _promote_retrieved_run_to_history(self, *, site_key: str, batch_id: str) -> None:
        self.applications.promote_retrieved_run_to_history(site_key=site_key, batch_id=batch_id)

    def _promote_apply_run_to_history(self, *, site_key: str, batch_id: str, session_id: str, turn_id: str) -> None:
        self.applications.promote_apply_run_to_history(
            site_key=site_key,
            batch_id=batch_id,
            session_id=session_id,
            turn_id=turn_id,
