...
from careereng.orchestration.context import BrowserPhaseMemory
  ...
  return {
    ...
    def _user_prompt(
        ...
        *,
        ...
        guidance = (
            phase_context.combined_guidance
            if phase_context is not None and phase_context.combined_guidance
            else phase.combined_guidance
        ) or "No phase-specific guidance was found."
        ...
        if phase_context is not None:
            if phase_context.local_state:
                local_state_line = f"Current local phase state:\n{json.dumps(phase_context.local_state, ensure_ascii=False, sort_keys=True)}\n\n"
            if phase_context.continuation:
                continuation_line = (
                    "Continuation context from a resumed session:\n"
                    f"{json.dumps(phase_context.continuation, ensure_ascii=False, sort_keys=True)}\n\n"
                )
        return (
            f"Site: {site_name}\n"
            f"Phase: {phase.title}\n"
    ...
    def _request_context_payload(*, context_session: Any | None, arguments: dict[str, Any]) -> dict[str, Any]:
        return execute_state_tool(
            "request_context",
            arguments,
            PhaseStateToolContext(site_store=None, site_key="", context_session=context_session),
        )
    ...
    def _update_phase_memory_payload(cls, *, phase_memory: BrowserPhaseMemory | None, arguments: dict[str, Any]) -> dict[str, Any]:
        return execute_state_tool(
            "update_phase_memory",
            arguments,
            PhaseStateToolContext(site_store=None, site_key="", phase_memory=phase_memory),
        )
    ...
    async def run_phase(
        ...
        *,
        ...
        active_phase_context = phase_context or build_phase_context(
            phase,
            phase_memory=phase_memory,
            local_state={"site_key": site_key, "batch_id": batch_id, "turn_id": turn_id},
        )
            ...
            base_items: list[dict[str, Any]] = [
                {"role": "system", "content": self._system_prompt(site_name=site_name, phase=phase)},
                {
                    ...
                    "content": self._user_prompt(
                        ...
                        entry_url=entry_url,
                        phase=phase,
                        phase_context=active_phase_context,
