...
class DomainStore:
    ...
    def save_doc(self, data: dict[str, Any], reason: str) -> None:
        if self.doc_path.exists():
            snapshot_name = f"{self.doc_path.stem}_{now_iso().replace(':', '-')}.md"
            (self.history_dir / snapshot_name).write_text(self.doc_path.read_text(encoding="utf-8"), encoding="utf-8")
        data = dict(data)
        data["updated_at"] = now_iso()[:10]
        self.doc_path.write_text(dump_front_matter(data), encoding="utf-8")
        self.append_event(
            {
                "name": "doc.updated",
                "related": True,
