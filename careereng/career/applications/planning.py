...
class ApplicationPlanningService:
    ...
    def apply_counter_payload(counters: dict[str, int]) -> dict[str, int]:
        return {
            ...
            "history_skipped": int(counters.get("history_skipped") or 0),
            "live_already_applied": int(counters.get("live_already_applied") or 0),
            "already_applied_unattributed": int(counters.get("already_applied_unattributed") or 0),
